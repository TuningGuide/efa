...
public class WettDefs extends DatenListe {
    ...
    try {
      ...
      while ( (s = freadLine()) != null) {
        ...
        if (block.equals("WETTDEF")) {
          ...
          if (s.startsWith("NAME=")) wettDef.name = s.substring(5,s.length());
          if (s.startsWith("KURZNAME=")) wettDef.kurzname = s.substring(9,s.length());
          if (s.startsWith("KEY=")) {
            wettDef.key = s.substring(4,s.length());
            if (wettDef.key.equals(STR_DRV_FAHRTENABZEICHEN)) wettDef.wettid = DRV_FAHRTENABZEICHEN;
            if (wettDef.key.equals(STR_DRV_WANDERRUDERSTATISTIK)) wettDef.wettid = DRV_WANDERRUDERSTATISTIK;
            if (wettDef.key.equals(STR_LRVBERLIN_SOMMER)) wettDef.wettid = LRVBERLIN_SOMMER;
            if (wettDef.key.equals(STR_LRVBERLIN_WINTER)) wettDef.wettid = LRVBERLIN_WINTER;
            if (wettDef.key.equals(STR_LRVBERLIN_BLAUERWIMPEL)) wettDef.wettid = LRVBERLIN_BLAUERWIMPEL;
            if (wettDef.key.equals(STR_LRVBRB_WANDERRUDERWETT)) wettDef.wettid = LRVBRB_WANDERRUDERWETT;
            if (wettDef.key.equals(STR_LRVBRB_FAHRTENWETT)) wettDef.wettid = LRVBRB_FAHRTENWETT;
            if (wettDef.key.equals(STR_LRVMVP_WANDERRUDERWETT)) wettDef.wettid = LRVMVP_WANDERRUDERWETT;
          }
          if (s.startsWith("GUELTIG_VON=")) wettDef.gueltig_von = EfaUtil.string2int(s.substring(12,s.length()),1);
          if (s.startsWith("GUELTIG_BIS=")) wettDef.gueltig_bis = EfaUtil.string2int(s.substring(12,s.length()),9999);
          if (s.startsWith("GUELTIG_PRIO=")) wettDef.gueltig_prio = EfaUtil.string2int(s.substring(13,s.length()),0);
          if (s.startsWith("VON=")) wettDef.von = EfaUtil.string2date(s.substring(4,s.length()),1,1,0);
          if (s.startsWith("BIS=")) wettDef.bis = EfaUtil.string2date(s.substring(4,s.length()),31,12,0);
          if (s.startsWith("GRUPPEN[")) {
            ...
            if (pos2>0) {
              ...
              pos1 = s.indexOf("[");
              pos2 = s.indexOf("]");
              if (pos1 == 0 && pos2>1) field = s.substring(pos1+1,pos2);
            }
            pos2 = s.indexOf("=");
            if (pos2>=0) value = s.substring(pos2+1,s.length()).trim();
            if (index >= 0 && field != null && field.length()>0 && value != null && value.length()>0) {
              if (gruppen == null) gruppen = new Vector();
              ...
              if (field.equals("GRUPPE")) g.gruppe = i_value;
              if (field.equals("UNTERGRUPPE")) g.untergruppe = i_value;
              if (field.equals("BEZEICHNUNG")) g.bezeichnung = value;
              if (field.equals("GESCHLECHT")) g.geschlecht = i_value;
              if (field.equals("HOECHSTALTER")) g.hoechstalter = i_value;
              if (field.equals("MINDALTER")) g.mindalter = i_value;
              if (field.equals("KM")) g.km = i_value;
              if (field.equals("ZUSATZ")) g.zusatz = i_value;
              if (field.equals("ZUSATZ2")) g.zusatz2 = i_value;
              if (field.equals("ZUSATZ3")) g.zusatz3 = i_value;
              if (field.equals("ZUSATZ4")) g.zusatz4 = i_value;
              if (field.equals("BEHINDERUNG")) g.behinderung = i_value;
            }
          }
        }
      }
  ...
  public synchronized boolean writeEinstellungen() {
    if (efw_letzte_aktualisierung == null) iniDefaultsForEFW();
    try {
      fwrite("\n[EFW]\n");
      ...
      for (int i=0; wettDefs != null && i<wettDefs.size(); i++) {
        ...
        if (w.name != null) fwrite("NAME="+w.name+"\n");
        if (w.kurzname != null) fwrite("KURZNAME="+w.kurzname+"\n");
        if (w.key != null) fwrite("KEY="+w.key+"\n");
        if (w.gueltig_von != -1) fwrite("GUELTIG_VON="+Integer.toString(w.gueltig_von)+"\n");
        if (w.gueltig_bis != -1) fwrite("GUELTIG_BIS="+Integer.toString(w.gueltig_bis)+"\n");
        if (w.gueltig_prio != -1) fwrite("GUELTIG_PRIO="+Integer.toString(w.gueltig_prio)+"\n");
        if (w.von != null) fwrite("VON="+w.von.tag+","+w.von.monat+","+w.von.jahr+"\n");
        if (w.bis != null) fwrite("BIS="+w.bis.tag+","+w.bis.monat+","+w.bis.jahr+"\n");
        for (int j=0; w.gruppen != null && j<w.gruppen.length; j++) {
          WettDefGruppe g = w.gruppen[j];
          if (g.gruppe != -1) fwrite("GRUPPEN["+j+"][GRUPPE]="+g.gruppe+"\n");
          if (g.untergruppe != -1) fwrite("GRUPPEN["+j+"][UNTERGRUPPE]="+g.untergruppe+"\n");
          if (g.bezeichnung.length() > 0) fwrite("GRUPPEN["+j+"][BEZEICHNUNG]="+g.bezeichnung+"\n");
          if (g.geschlecht != -1) fwrite("GRUPPEN["+j+"][GESCHLECHT]="+g.geschlecht+"\n");
          if (g.hoechstalter != -1) fwrite("GRUPPEN["+j+"][HOECHSTALTER]="+g.hoechstalter+"\n");
          if (g.mindalter != -1) fwrite("GRUPPEN["+j+"][MINDALTER]="+g.mindalter+"\n");
          if (g.km != -1) fwrite("GRUPPEN["+j+"][KM]="+g.km+"\n");
          if (g.zusatz != -1) fwrite("GRUPPEN["+j+"][ZUSATZ]="+g.zusatz+"\n");
          if (g.zusatz2 != -1) fwrite("GRUPPEN["+j+"][ZUSATZ2]="+g.zusatz2+"\n");
          if (g.zusatz3 != -1) fwrite("GRUPPEN["+j+"][ZUSATZ3]="+g.zusatz3+"\n");
          if (g.zusatz4 != -1) fwrite("GRUPPEN["+j+"][ZUSATZ4]="+g.zusatz4+"\n");
          if (g.behinderung != -1) fwrite("GRUPPEN["+j+"][BEHINDERUNG]="+g.behinderung+"\n");
        }
      }
    } catch(IOException e) {
      errWritingFile(dat);
  ...
  public WettDef getWettDef(int wettnr, int wettjahr) {
    ...
    if (found_nr >= 0) return (WettDef)wettDefs.get(found_nr);
    else return null;
  }


  public boolean inGruppe(int wettnr, int wettJahr, int gruppe, int jahrgang, int geschlecht, boolean behind) {
    if (wettnr == LRVBERLIN_BLAUERWIMPEL || wettnr == DRV_WANDERRUDERSTATISTIK) return true;
    if (wettnr < 0 || wettnr >= ANZWETT) return false;
    WettDef wett = getWettDef(wettnr,wettJahr);
    if (wett == null || wett.gruppen == null) return false;
    if (jahrgang<wettJahr-wett.gruppen[gruppe].hoechstalter ||
        jahrgang>wettJahr-wett.gruppen[gruppe].mindalter) return false;
    if (wett.gruppen[gruppe].geschlecht != 2 && geschlecht != wett.gruppen[gruppe].geschlecht) return false;
    if ( (behind && wett.gruppen[gruppe].behinderung==BEHIND_NEIN) || (!behind && wett.gruppen[gruppe].behinderung==BEHIND_JA) ) return false;
    return true;
  }
  ...
  public boolean erfuelltGruppe(int wettnr, int wettJahr, int gruppe, int jahrgang, int geschlecht, boolean behind, int km, int zusatz, int zusatz2, int zusatz3, int zusatz4) {
    if (wettnr == LRVBERLIN_BLAUERWIMPEL || wettnr == DRV_WANDERRUDERSTATISTIK) return true;
    if (jahrgang != 0 && !inGruppe(wettnr,wettJahr,gruppe,jahrgang,geschlecht,behind)) return false;
    WettDef wett = getWettDef(wettnr,wettJahr);
    if (wett == null || wett.gruppen == null) return false;
    if (km / 10 < wett.gruppen[gruppe].km) return false;
    switch (wettnr) {
      case DRV_FAHRTENABZEICHEN: // zusatz:= wafaKm/10        zusatz2:= wafaAnzMTour     zusatz3:= jumAnz
        if ( !((wett.gruppen[gruppe].gruppe <  3 && zusatz >= wett.gruppen[gruppe].zusatz) || // Gruppe 1/2: WafaKm erfüllt?
               (wett.gruppen[gruppe].gruppe == 3 && (zusatz2>=3 ||            // Gruppe 3: Anz Wafa erfüllt?
                          wett.gruppen[gruppe].untergruppe<=2 && (zusatz2==2 && zusatz3>=2 || // oder bei a/b mit Hilfe von JuM erfüllt?
                                                              zusatz3>=4) ))) ) return false;
        break;
      case LRVBERLIN_SOMMER:     // zusatz:= <Zielfahrtenanzahl>
        if (zusatz < wett.gruppen[gruppe].zusatz) return false;
        break;
      case LRVBERLIN_WINTER:     // zusatz:= winterAnz   zusatz2:= anzMonate
        if (zusatz < wett.gruppen[gruppe].zusatz || zusatz2 < wett.gruppen[gruppe].zusatz2) return false;
        break;
      ...
      case LRVMVP_WANDERRUDERWETT: // zusatz:= Gig-Km; zusatz2:= Gig-Fahrten; zusatz3:= 20Km-Fahrten; zusatz4:= 30Km-Fahrten
        if (zusatz < wett.gruppen[gruppe].zusatz || zusatz2 < wett.gruppen[gruppe].zusatz2 ||
            zusatz3 < wett.gruppen[gruppe].zusatz3 || zusatz4 < wett.gruppen[gruppe].zusatz4) return false;
    }
    return true;
  }
  ...
  public String erfuellt(int wettnr, int wettJahr, int jahrgang, int geschlecht, boolean behind, int km, int zusatz, int zusatz2, int zusatz3, int zusatz4) {
    if (wettnr == LRVBERLIN_BLAUERWIMPEL || wettnr == DRV_WANDERRUDERSTATISTIK) return ""; // erfüllt
    WettDef wett = getWettDef(wettnr,wettJahr);
    if (wett == null || wett.gruppen == null) return null;
  ...
  public static String getDRVAbzeichen(boolean erwachsen, int abzeichen, int abzeichenAB, int wettJahr) {
    ...
    if (wettJahr < 2007) abzeichenGesamt = abzeichen - abzeichenAB;
    if (erwachsen) {
      // Erwachsene (Gruppe 1 oder 2)
      if ( (abzeichenGesamt + 1) >= 5 && (abzeichenGesamt + 1) % 5 == 0 ) return EfaWettMeldung.getAbzeichenGold(abzeichenGesamt + 1,true);
      ...
      if ( (abzeichenGesamt + 1) >= 5 && (abzeichenGesamt + 1) % 5 == 0 ) return EfaWettMeldung.getAbzeichenGold(abzeichenGesamt + 1,true); // Erw-Gold auch für Jugendliche!
      if ( (abzeichen + 1) >= 5 && (abzeichen + 1) % 5 == 0) return EfaWettMeldung.getAbzeichenGold(abzeichen + 1,false);
