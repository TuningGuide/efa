...
public class VereinsConfigFrame extends JDialog implements ActionListener {
  ...
  private void jbInit() throws Exception {
    ...
    userDrvLabel.setText("Benutzername f�r efaWett (DRV): ");
    ...
    jLabel11.setText("Versandanschrift f�r Meldeergebnisse etc.");
    versandNameLabel.setText("Name: ");
    versandStrasseLabel.setText("Stra�e, Hausnr: ");
    ...
    userLrvLabel.setText("Benutzername f�r efaWett (LRV): ");
    ...
    jLabel4.setText("efaWett - elektronisches Meldesystem f�r Wettbewerbe");
    jLabel7.setText("    - F�r die elektronische Meldung anmelden");
    jLabel8.setText("    - Wettbewerbsmeldungen einsenden");
    jLabel9.setText("    - Status der letzten Wettbewerbsmeldung abfragen");
    jLabel10.setText("    - In efaWett gespeicherte Vereinsdaten einsehen/�ndern");
    efaWettHomepageButton.setText("efaWett-Homepage �ffnen");
  ...
  public void actionPerformed(ActionEvent e) {
   ...
   void iniFelder(VereinsConfig vc) {
    ...
    info.append("Die folgenden Daten werden zum Erstellen von elektronischen Wettbewerbsmeldungen ben�tigt.\n"+
                "Vereinsdaten: Daten des Vereins, die f�r Wettbewerbsmeldungen ben�tigt werden\n"+
                "efa-Konfiguraion: Konfiguration von efa f�r elektronische Wettbewerbsmeldungen aktualisieren\n"+
                "efa-Wett: F�r elektronische Wettbewerbsmeldung anmelden, Status von Meldungen abrufen etc.");
    efwErklaerung.append("efa speichert diverse Konfigurationswerte, die f�r die elektronische Meldung\n"+
                         "erforderlich sind. Hierzu z�hlen insbesondere Adressen zum elektronischen\n"+
                         "Einsenden der Daten sowie Informationen �ber die anfallenden Meldegeb�hren.\n"+
    ...
    efaWettInfo.append("efaWett ist ein elektronisches Meldesystem im Internet zum Einsenden von\n"+
                       "Wettbewerbsmeldungen. Um Wettbewerbe elektronisch melden zu k�nnen, mu� sich\n"+
                       "jeder Verein zun�chst bei efaWett kostenlos anmelden. Anschlie�end kann der\n"+
                       "Verein f�r die angebotenen Wettbewerbe mit Hilfe von efa elektronisch melden.\n"+
   ...
   void iniFelder(EfaWett ew) {
    ...
    info.append("Die folgenden Daten sind f�r die Meldung erforderlich.\n"+
                "Bitte �berpr�fe (und vervollst�ndige ggf.) die Angaben und klicke anschlie�end OK.");
    efwErklaerung.append("efa speichert diverse Konfigurationswerte, die f�r die elektronische Meldung\n"+
                         "erforderlich sind. Hierzu z�hlen insbesondere Adressen zum elektronischen\n"+
                         "Einsenden der Daten sowie Informationen �ber die anfallenden Meldegeb�hren.\n"+
                         "Damit diese Daten stets korrekt sind, sollten sie jeweils vor der Erstellung einer\n"+
  ...
  boolean checkNotEmpty(JTextField t) {
    if (t.isVisible() && t.getText().trim().length()==0) {
      Dialog.error("Bitte f�lle alle angezeigten Felder vollst�ndig aus!");
      t.requestFocus();
      return false;
    }
    return true;
  ...
  void okButton_actionPerformed(ActionEvent e) {
    ...
    if (!efwErstellen) {
      ...
      cancel();
    } else { // Daten f�r efa-Wett erstellen
  ...
  private static boolean holeAktuelleDatenManuell(boolean autoDownloadHasFailed) {
    ...
    if (autoDownloadHasFailed) {
      if (Dialog.yesNoCancelDialog("Wettbewerbskonfiguration manuell aktualisieren",
                                   ...
                                   "fehlgeschlagen. M�glicherweise wurde der Zugriff von efa durch eine Firewall\n"+
                                   "blockiert. Du kannst nun mit einem Webbrowser die Konfigurationsdaten manuell\n"+
                                   "herunterladen. M�chtest Du die Konfigurationsdaten jetzt manuell herunterladen?") != Dialog.YES) return false;
    }
    Dialog.meldung("Konfigurationsdatei herunterladen",
                   ...
                   "Klicke anschlie�end bitte OK und w�hle im folgenden Dialog die soeben\n"+
                   "heruntergeladene Datei aus.");
    String datei = Dialog.dateiDialog(dialog,"Konfigurationsdatei ausw�hlen","Wettbewerbskonfiguration (*.data)","data",null,false);
    if (datei == null) return false;
    if (!EfaUtil.canOpenFile(datei)) {
      Dialog.error("Die Datei "+datei+" kann nicht ge�ffnet werden.");
      return false;
    }
