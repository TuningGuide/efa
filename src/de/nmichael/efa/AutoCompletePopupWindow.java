...
public class AutoCompletePopupWindow extends JWindow {
  ...
  private void showAtTextField(JTextField field) {
    if (showingAt == field) {
      // Unter Windows bewirkt toFront(), daﬂ der urspr¸ngliche Frame den Fokus verliert, daher muﬂ unter Windows darauf verzichtet werden
    ...
    try {
      ...
      if (!Daten.osName.startsWith("Windows")) this.toFront();
      showingAt = field;
    } catch(Exception ee) { // nur zur Sicherheit: Es gibt seltene Exceptions in efa, die keiner Stelle im Code zugeordnet werden kˆnnen und hierher kommen kˆnnten
    }
  }
