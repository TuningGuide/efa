...
public class EnterPasswordFrame extends JDialog implements ActionListener {
  ...
  private void jbInit() throws Exception {
    ...
    this.setTitle("Paﬂworteingabe");
    grund.setBackground(Color.lightGray);
    grund.setEditable(false);
    passwort2Label.setText("Paﬂwort (Wiederholung): ");
    jLabel1.setText(" ");
  ...
  void okButton_actionPerformed(ActionEvent e) {
    ...
    if (newPwd) {
      ...
      if (!pwd.equals(pwd2)) {
        Dialog.error("Die beiden eingegebenen Paﬂwˆrter sind verschieden.\nDu muﬂt in beide Felder dasselbe Paﬂwort eingeben!");
        passwort2.requestFocus();
        return;
      }

      if (pwd.indexOf(" ") >= 0) {
        Dialog.error("Das Paﬂwort darf keine Leerzeichen enthalten!");
        passwort.requestFocus();
        return;
      }
      if (pwd.length()<8) {
        Dialog.error("Das Paﬂwort muﬂ mindestens 8 Zeichen lang sein!");
        passwort.requestFocus();
        return;
      }
      ...
      if (merkmale<3) {
        Dialog.error("Das Paﬂwort muﬂ mindestens Zeichen aus drei der insgesamt vier Zeichengruppen\n"+
                     "'Kleinbuchstaben', 'Groﬂbuchstaben', 'Ziffern' und 'sonstige Zeichen' enthalten!");
        passwort.requestFocus();
        return;
      }
    }
