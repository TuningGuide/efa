...
public class DRVSignatur {
  ...
  public DRVSignatur(String fahrtenheft) {
    ...
    // Signatur ermitteln
    if (v.size()>0) this._signatur = EfaUtil.replace((String)v.get(v.size()-1)," ","",true); // _signatur ist immer das letzte Element

    // Signatur, Version und Schlüssel ermitteln
    if (_signatur != null) {
      byte[] plainsig = Base64.decode(_signatur);
      if (plainsig.length>=3) {
        this.version = plainsig[0];
        this.keyNr = plainsig[1];
        this.signatur = new byte[plainsig.length-2];
        for (int i=0; i<this.signatur.length; i++) this.signatur[i] = plainsig[i+2];
      }
    }
    ...
    for (int i=0; i<v.size(); i++) {
      switch(this.version) {
        case 2: // Version 2
          switch(i) {
            case  0: this.teilnNr = (String)v.get(i); break;
            case  1: this.vorname = (String)v.get(i); break;
            case  2: this.nachname = (String)v.get(i); break;
            case  3: this.jahrgang = (String)v.get(i); break;
            case  4: this.anzAbzeichen = EfaUtil.string2int((String)v.get(i),0); break;
            case  5: this.gesKm = EfaUtil.string2int((String)v.get(i),0); break;
            case  6: this.anzAbzeichenAB = EfaUtil.string2int((String)v.get(i),0); break;
            case  7: this.gesKmAB = EfaUtil.string2int((String)v.get(i),0); break;
            case  8: this.jahr = EfaUtil.string2int((String)v.get(i),0); break;
            case  9: this.sigDatum = (String)v.get(i); break;
          }
          break;
        case 3: // Version 3
          switch(i) {
            ...
            case  7: this.gesKmAB = EfaUtil.string2int((String)v.get(i),0); break;
            case  8: this.jahr = EfaUtil.string2int((String)v.get(i),0); break;
            case  9: this.letztkm = EfaUtil.string2int((String)v.get(i),0); break;
            case 10: this.sigDatum = (String)v.get(i); break;
          }
          break;
  ...
  public DRVSignatur(String teilnNr, String vorname, String nachname, String jahrgang, int anzAbzeichen,
    ...
    if (teilnNr == null || teilnNr.length()==0) throw new Exception("Ungültige Teilnehmernummer");
    if (vorname == null || vorname.length()==0) throw new Exception("Ungültiger Vorname");
    if (nachname == null || nachname.length()==0) throw new Exception("Ungültiger Nachname");
    if (jahrgang == null || jahrgang.length()!=4) throw new Exception("Ungültiger Jahrgang");
    if (anzAbzeichen <= 0) throw new Exception("Ungültige Anzahl an Abzeichen");
    if (gesKm <= 0) throw new Exception("Ungültige Anzahl an Kilometern");
    if (anzAbzeichenAB < 0) throw new Exception("Ungültige Anzahl an Abzeichen der Jugeng-Gruppen A/B");
    if (gesKmAB < 0) throw new Exception("Ungültige Anzahl an AB-Kilometern der Jugeng-Gruppen A/B");
    if (anzAbzeichenAB > anzAbzeichen) throw new Exception("Anzahl der Abzeichen für Jugend-Gruppen A/B ist größer als Anzahl der Abzeichen insgesamt");
    if (gesKmAB > gesKm) throw new Exception("Anzahl der Kilometer für Jugend-Gruppen A/B ist größer als Anzahl der Kilometer insgesamt");
    if (jahr < 1900) throw new Exception("Ungültiges Erfüllungsjahr");
    if (letztkm < 0) throw new Exception("Ungültige Kilometer für letztes Erfüllungsjahr");
    if (version < VERSION_MIN || version > VERSION_MAX) throw new Exception("Ungültige Version");
    if (keyNr < 0) throw new Exception("Ungültige Schlüssel-Nummer");
    if (privKey == null)  throw new Exception("Ungültiger Schlüssel (Schlüssel nicht gefunden)");
    ...
    try {
      ...
      keySig[1] = this.keyNr;
      for (int i=0; i<this.signatur.length; i++) keySig[i+2] = this.signatur[i];
  ...
  public Date getSignatureDate() {
    if (this.sigDatum == null) return null;
    if (this.sigDatum.length() != 12) return null;
    try {
  ...
  private String construct_meldung(boolean withBlanks) {
    switch(this.version) {
      case 2: // Version 2
        if (withBlanks) return teilnNr + "; " + vorname + "; " + nachname + "; " + jahrgang + "; " + anzAbzeichen + "; " + gesKm + "; " + anzAbzeichenAB + "; " + gesKmAB + "; " + jahr + "; " + sigDatum;
        return teilnNr + ";" + vorname + ";" + nachname + ";" + jahrgang + ";" + anzAbzeichen + ";" + gesKm + ";" + anzAbzeichenAB + ";" + gesKmAB + ";" + jahr + ";" + sigDatum;
      case 3: // Version 3
        if (withBlanks) return teilnNr + "; " + vorname + "; " + nachname + "; " + jahrgang + "; " + anzAbzeichen + "; " + gesKm + "; " + anzAbzeichenAB + "; " + gesKmAB + "; " + jahr + "; " + letztkm + "; " + sigDatum;
  ...
  private X509Certificate getCertificate() {
    if (Daten.keyStore == null) return null;
    return Daten.keyStore.getCertificate(getKeyName());
  }

  public String toString() {
    if (_meldung == null || _signatur == null) return null;
  ...
  public String getSignaturString() {
    if (_signatur == null) return _signatur;
    String s = "";
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      sig.checkSignature();
      if (sig.getSignatureState() == SIG_VALID) System.out.println("Signatur ist gültig!");
      else System.out.println("Signatur ist ungültig: "+sig.getSignatureError());
    } catch(Exception e) {
      e.printStackTrace();
    }
  }
