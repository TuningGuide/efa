 ...
 */
 ...
 component
   ...
   */
   ...
   {
     int iLength=-1;
     if (aObj.length == 2 && aObj[0].length > 0 && aObj[0].length == aObj
 [1].length)
     ...
     for (int i=0; i<iLength; i++)
     {
       ...
       {
         Method m=methodObject.getClass().getDeclaredMethod(routineName,new
 ...
 [1].length)
     ...
     for (int i=0; i<iLength; i++)
     {
       if (aObj[0][i] instanceof JButton)
         ...
         ((AbstractButton)aObj[0][i]).setAction((AbstractAction) aObj[1]
 [i]);
       else if (aObj[0][i] instanceof JTextField)
 ...
 component
    ...
    @throws NoSuchMethodException     -if the method is not found.
   */
   public void addKeyActions(JComponent comp, int inputcondition, String[]
 keystrokes, String[] methodNames)
   throws IllegalArgumentException, NoSuchMethodException
   {
