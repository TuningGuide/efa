...
public class StatistikFrame extends JDialog implements ActionListener {
  ...
  // Achtung: Diese Werte m�ssen mit den AUSGABE_* Konstanten aus StatistikDaten �bereinstimmen!
  final static String[] ausgabeArten =
    { "im Programm (Grafik)", // 0
      ...
      "als Textdatei",        // 6
      "als CSV-Datei" };      // 7
//    "als Meldedatei"        // 8 (wird dynamisch hinzugef�gt, wenn WettPanel aktiv ist)
  final static String[] ausgabeExt =
    { null, null, null, "html", "pdf", "xml", "txt", "csv", null };

  String aktStatName = ""; // "" normalerweise bzw. Name der bearbeiteten Statistik (wenn "Bearbeiten" gew�hlt
  boolean aktStatAuchEfaDirekt = false; // ob aktuell bearbeitete Statistik auch in efaDirekt verf�gbar sein soll
  ...
  private void jbInit() throws Exception {
    ...
    zeitraumLabel.setText("Zeitraum f�r die Berechnung: ");
    ...
    auswahlLabel.setText("nur Fahrten berechnen f�r:");
    ...
    nurName.setToolTipText("nur Daten f�r diese Person auswerten (bzw. f�r alle Personen, in " +
    ...
    auswahlPanel.setToolTipText("Auswertung auf bestimmte Kriterien einschr�nken: Nur Ruderer/Innen, " +
    "auf die eines der Merkmale pa�t, werden bei der Auswertung ber�cksichtigt.");
    ...
    maxBalkenKm.setToolTipText("legt die Gr��e des Balkens f�r 100% fest (Angabe in Pixeln)");
    ...
    maxBalkenRudKm.setToolTipText("legt die Gr��e des Balkens f�r 100% fest (Angabe in Pixeln)");
    ...
    maxBalkenStmKm.setToolTipText("legt die Gr��e des Balkens f�r 100% fest (Angabe in Pixeln)");
    ...
    maxBalkenFahrten.setToolTipText("legt die Gr��e des Balkens f�r 100% fest (Angabe in Pixeln)");
    ...
    maxBalkenKmFahrt.setToolTipText("legt die Gr��e des Balkens f�r 100% fest (Angabe in Pixeln)");
    ...
    "werden nur Eintr�ge ausgewertet, f�r die der Name exakt �bereinstimmt.");
    ...
    bNurLabel.setText("nur Fahrten berechnen f�r:");
    ...
    bNurAnzahlLabel.setText("Ruderpl�tze:");
    ...
    mNurBoot.setToolTipText("nur Daten f�r dieses Boot auswerten");
    ...
    bNurPanel.setToolTipText("Auswertung auf bestimmte Kriterien einschr�nken: Nur Boote, auf die " +
    "eines der Merkmale pa�t, werden bei der Auswertung ber�cksichtigt.");
    ...
    mGraSizeLabel.setText("Gr��e:");
    mGraSizeKmFahrt.setNextFocusableComponent(mGraAusDauer);
    mGraSizeKmFahrt.setPreferredSize(new Dimension(40, 19));
    mGraSizeKmFahrt.setToolTipText("legt die Gr��e des Balkens f�r 100% fest (Angabe in Pixeln)");
    ...
    mGraSizeFahrten.setToolTipText("legt die Gr��e des Balkens f�r 100% fest (Angabe in Pixeln)");
    ...
    mGraSizeKm.setToolTipText("legt die Gr��e des Balkens f�r 100% fest (Angabe in Pixeln)");
    ...
    mNumPanel.setToolTipText("legt fest, welche Eintr�ge numeriert werden sollen und welche nicht");
    ...
    numerierung.setToolTipText("legt fest, welche Eintr�ge numeriert werden sollen und welche nicht");
    ...
    zeitFbUebergreifend.setText("fahrtenbuch�bergreifend");
    zeitFbUebergreifend.setNextFocusableComponent(auswahlPane);
    zeitFbUebergreifend.setToolTipText("alle Fahrtenb�cher, die miteinander verkn�pft sind, einbeziehen");
    ...
    wettAnzLabel.setToolTipText("alle Ruderer/Innen ausgeben, die diese Anzahl von Fahrten erf�llt " +
    "haben");
    ...
    wettAnz.setToolTipText("alle Ruderer/Innen ausgeben, die diese Anzahl von Fahrten erf�llt " +
    "haben");
    wettAnz.setText("4");
    ...
    wettTitleLabel.setText("Ausgabe nur, wenn mindestens erf�llt sind:");
...
*/
  ...
  public void itemsIni() {
    ...
    if (Daten.wettDefs != null) {
      for (int i=0; i<WettDefs.ANZWETT; i++) {
        ...
        if (w != null) wettArt.addItem(w.name); // Namen der Wettbewerbe f�r h�chstm�gliches Jahr (falls sich Namen �ndern)!
      }
  ...
  // Auswahl der Statistikart ge�ndert --> Listen anpassen
...
//      rudererOptionenPanel.setVisible(true);
  ...
  // Auswahlliste Art (Boote) ge�ndert
...
//      booteOptionenPanel.setVisible(true);
  ...
  // Sortierkriterium ge�ndert --> ggf. Sortierreihenfolge anpassen
  ...
  // zu aktuellem Datum das beste passende Wettbewerbsjahr f�r einen gegebenen Wettbewerb ermitteln
  ...
  void getWettJahr() {
    ...
    wett = Daten.wettDefs.getWettDef(wettArt.getSelectedIndex(),year); // anhand des ermittelten Jahres jetzt nochmal die g�ltigen Wettbewerbsbedinungen holen
    if (wett == null) return;
  ...
  // Wettbewerbsjahr vervollst�ndigen
  ...
  void wettbewerbPanel_componentShown(ComponentEvent e) {
    ...
    if (!statWettFromSavedValues)
      nurStatus1.setSelectionInterval(0,Daten.fahrtenbuch.getDaten().status.length-3); // Gast standardm��ig nicht ausgeben
  ...
  void ausgabeDateiButton_actionPerformed(ActionEvent e) {
    ...
    String dat;
    if (!ausgabeDatei.getText().trim().equals("")) dat = Dialog.dateiDialog(this,ext.toUpperCase()+"-Datei ausw�hlen",ext.toUpperCase()+"-Datei (*."+ext+")",ext,ausgabeDatei.getText().trim(),null,"Datei ausw�hlen",true,false);
    else dat = Dialog.dateiDialog(this,ext.toUpperCase()+"-Datei ausw�hlen",ext.toUpperCase()+"-Datei (*."+ext+")",ext,null,null,"Datei ausw�hlen",true,false);
  ...
  void erweitertButton_actionPerformed(ActionEvent e) {
    ...
    // je nach aktuell ausgew�hlter Statistik die Elemente des erweitertFrame ggf. in grau darstellen
  ...
  // Ermittelt, ob eine Output-Datei, die f�r d erzeugt werden soll, geschrieben, ggf. �berschrieben, werden darf
  ...
  void speichernButton_actionPerformed(ActionEvent e) {
    ...
    if (d.ausgabeArt == StatistikDaten.AUSGABE_EFAWETT) {
      Dialog.infoDialog("Fehler","Meldedatei-Ausgaben k�nnen nicht als Statistikeinstellung gespeichert werden!");
      return;
    }
    ...
    do {
      ...
      if (f != null) {
        ...
        switch (art = EfaUtil.string2int(f.get(StatSave.ART),StatistikDaten.ART_MITGLIEDER)) {
            ...
            case StatistikDaten.ART_MONATSUEBERSICHT: felder[i] = felder[i] + "Monats�bersicht)"; break;
            ...
            case StatistikDaten.BART_PLAETZE: felder[i] = felder[i] + "Bootspl�tze)"; break;
  ...
  // gespeicherten Eintrag l�schen
  void deleteButton_actionPerformed(ActionEvent e) {
    if (statList.getSelectedIndices().length == 0) return;
    switch (Dialog.yesNoDialog("Warnung","Sollen die markierten Eintr�ge wirklich gel�scht werden?")) {
      case Dialog.YES:
        String[] s = new String[statList.getSelectedValues().length];
        for (int i=0; i<statList.getSelectedValues().length; i++)
          s[i] = (String)statList.getSelectedValues()[i];
        ...
        else Dialog.error("�nderungen konnten nicht gespeichert werden!");
        showSavedStat();
        break;
  ...
  // boolean-Array zu int-Array machen f�r setSelectedIndices
  ...
  // Fehler, wenn eines der "nur ..."-Felder komplett leer ist (d.h. keine Eintr�ge ausgew�hlt wurden!)
  boolean errorIfEmpty(boolean[] b, String s) {
    boolean empty = true;
    for (int i=0; i<b.length; i++) if (b[i]) empty = false;
    if (empty)
      Dialog.infoDialog("Ungen�gende Auswahl","In der Auswahl '"+s+"' wurde kein Eintrag ausgew�hlt.\nBitte w�hle zumindest einen Eintrag aus!");
    return empty;
  }
  ...
  void doErstellen() {
    ...
    if (d[0].stat == StatistikDaten.STAT_BOOTE) {
      ...
      if (errorIfEmpty(d[0].bAnzahl,"Ruderpl�tze")) return;
      if (errorIfEmpty(d[0].fahrtart,"Art der Fahrt")) return;
  ...
  StatistikDaten createStatistik() {
    ...
    if (d.ausgabeArt == StatistikDaten.AUSGABE_PDF && ausgabeFormat.getSelectedIndex()==0) {
      Dialog.infoDialog("Bitte w�hle eine Formatierung aus!","Keine Formatierung ausgew�hlt");
      return null;
    }
  ...
  // spezielle Daten f�r zus�tzliche Wettbewerbsausgabe
  ...
  // spezielle Daten f�r Mitglieder-Statistik ermitteln
  boolean createStatRuderer(StatistikDaten d) {
    if (art.getSelectedIndex() == StatistikDaten.ART_MITRUDERER &&
      ...
      Dialog.infoDialog("Fehler","Wenn Statistikart 'Mitruderer' gew�hlt ist, mu� bei 'nur Name'\n"+
  ...
  // spezielle Daten f�r Bootsstatiktik ermitteln
  ...
  boolean createStatWett(StatistikDaten d) {
    ...
    if (d.ausgabeArt == StatistikDaten.AUSGABE_EFAWETT &&
      ...
      Dialog.error("Die Ausgabe als Meldedatei wird f�r diesen Wettbewerb nicht unterst�tzt.");
      return false;
    }
    ...
    if (d.ausgabeArt == StatistikDaten.AUSGABE_EFAWETT) {
      ...
      if (!Statistik.checkWettZeitraum(d.wettJahr,d.von,d.bis,d.art-200)) {
        Dialog.infoDialog("Ung�ltiger Zeitraum","Der gew�hlte Zeitraum entspricht nicht den Bedingungen der Ausschreibung");
        return false;
      }
    }
  ...
  void setSavedStat(DatenFelder f, boolean defaultStat) {
    if (f != null) {
      try {
        ...
        // h��licher Workaround, damit wettPanel wei�, da� es nicht angeklickt wurde, sondern als gespeicherte Einstellungen
  ...
  void doEdit() {
    ...
    if (statList.getSelectedIndices().length>1) {
      Dialog.infoDialog("Zu viele Eintr�ge markiert","Es kann immer nur eine Statistikeinstellung bearbeitet werden!");
      return;
    }
    String s = (String)statList.getSelectedValue();
    if (s.lastIndexOf(" (")<0) {
      Dialog.error("Statistikdatei hat ung�ltiges Format!");
      return;
    }
  ...
  // allgemeine Einstellungen zur Statistik, die nicht aus der Eingabe gewonnen werden k�nnen
  ...
  public void efaWettVervollstaendigen(EfaWett efaWett) {
    ...
    if (efaWett.meldung == null) {
      Dialog.infoDialog("Keine Meldungen","Im gew�hlten Zeitraum haben keine Teilnehmer die Bedingungen erf�llt!");
      return;
    }
  ...
  void setAusgabeFormat(int nr) {
    ...
    if (ausgabeExt[nr] != null && ausgabeExt[nr].equals("pdf"))
      ausgabeFormat.addItem("--- bitte w�hlen ---");
    else
