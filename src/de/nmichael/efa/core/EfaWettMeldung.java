...
public class EfaWettMeldung {
  ...
  public static String getAbzeichenGold(int jahre, boolean erwachsene) {
    if (jahre % 5 != 0) return null;
    if (jahre < 5) return null;
    if (erwachsene) return ABZEICHEN_ERW_GOLD_LIST[jahre / 5];
    else return ABZEICHEN_JUG_GOLD_LIST[jahre / 5];
  }

  public static int getAbzeichenGoldIndex(String abzeichen) {
    for (int i=1; i<ABZEICHEN_ERW_GOLD_LIST.length; i++) if (ABZEICHEN_ERW_GOLD_LIST[i].equals(abzeichen)) return i;
    for (int i=1; i<ABZEICHEN_JUG_GOLD_LIST.length; i++) if (ABZEICHEN_JUG_GOLD_LIST[i].equals(abzeichen)) return i;
    return -1;
  }
